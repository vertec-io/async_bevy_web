...
impl Plugin for FramepacePlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(PreUpdate, get_display_refresh_rate);
...
fn get_display_refresh_rate(
    ...
    let new_frametime = match settings.limiter {
        Limiter::Auto => match detect_frametime() {
...
#[cfg(not(target_arch = "wasm32"))]
fn detect_frametime() -> Option<Duration> {
    // Just remove all logic and set this to be 1 micro second. In the future this 
    // could be something fancy that checks the system clock or CPU usage
    let best_frametime = Duration::from_micros(1);
    Some(best_frametime)
}
