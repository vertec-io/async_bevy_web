...
fn setup(mut server: ResMut<WebServer>){
    ...
    let listener = tokio::net::TcpListener::bind(&server.address);//.await.expect("Could not create TCP Listener");

    
    let router: Router<Arc<WebServer>> = Router::new()
    .route("/", get(root))
